...
public class Main {
	...
	private void printMedianMileageByYearOfProduction(int startingYear, int endYear){
		...
		System.out.println("Mediana przebiegu samochodów wyprodukowanych w latach " + startingYear + "-"+ endYear +":");
		DescriptiveStatistics stats = new DescriptiveStatistics();
		for(Car c: cars){
	...
	private void printDailyMeanMileageOfCarsMadeInYear(int year){
		...
		for(String b: brands){
			...
			for(Car c: carsByYear){
				if(c.getBrand().equals(b)) carsInCurrentBrand.add(c);
			}
			if(carsInCurrentBrand.size() == 0 ) continue;
